...
import com.old.silence.content.infrastructure.ollama.OllamaProperties;
 ...
 */
@Service
public class PoetryQuizQuestionGenerationService {

    private static final Logger log = LoggerFactory.getLogger(PoetryQuizQuestionGenerationService.class);
    ...
    public PoetryQuizQuestionGenerationService(
            OllamaService ollamaService,
            PoetryLearningContentRepository poetryLearningContentRepository,
            PoetryQuizQuestionsRepository poetryQuizQuestionsRepository,
     ...
     */
    @Transactional
    public int generateQuestionsForAllContents(Pageable pageable) {
        log.info("开始批量生成题目...");
        
        // 分页查询所有学习内容，pageNo=1, pageSize=1000
        Page<PoetryLearningContent> page = poetryLearningContentRepository.findByCriteria(
                null, pageable, PoetryLearningContent.class);
        
        int totalGenerated = 0;
        List<PoetryLearningContent> contents = page.getContent();
        log.info("找到 {} 条学习内容", contents.size());
        
        for (PoetryLearningContent content : contents) {
            try {
                int count = generateQuestionsForContent(content);
                totalGenerated += count;
                log.info("为内容 ID: {} (标题: {}) 生成了 {} 道题目", 
                        content.getId(), content.getTitle(), count);
            } catch (Exception e) {
                log.error("为内容 ID: {} 生成题目时出错: {}", content.getId(), e.getMessage(), e);
            }
        }
        
        log.info("批量生成完成，共生成 {} 道题目", totalGenerated);
     ...
     */
    @Transactional
    public int generateQuestionsForContent(BigInteger contentId) {
        PoetryLearningContent content = poetryLearningContentRepository
                .findById(contentId, PoetryLearningContent.class)
     ...
     */
    @Transactional
    public int generateQuestionsForContent(PoetryLearningContent content) {
        ...
        try {
            String model = ollamaProperties.getGenerationModel();
            log.debug("使用模型: {} 生成题目", model);
            generatedText = ollamaService.generateText(prompt, model);
            log.debug("Ollama 生成结果: {}", generatedText);
        } catch (Exception e) {
            log.error("调用 Ollama 生成题目失败: {}", e.getMessage(), e);
        ...
        try {
            questions = parseGeneratedQuestions(generatedText);
        } catch (Exception e) {
        ...
        for (QuestionData questionData : questions) {
            try {
                PoetryQuizQuestions question = convertToEntity(content.getId(), questionData);
                poetryQuizQuestionsRepository.create(question);
                savedCount++;
            } catch (Exception e) {
     ...
     * 构建提示词
     */
    private String buildPrompt(PoetryLearningContent content) {
    ...
    private List<QuestionData> parseGeneratedQuestions(String generatedText) {
        try {
            ...
            // 尝试解析完整的 JSON
            try {
                List<QuestionData> allQuestions = objectMapper.readValue(jsonText, new TypeReference<List<QuestionData>>() {});
                // 只取前5道题目，并去重
        ...
        while (currentPos < jsonText.length() && questions.size() < 5) {
            ...
            if (c == '{') {
                ...
                if (braceCount == 0) {
                    // 找到一个完整的对象
                    try {
                        String objJson = "[" + currentObject.toString() + "]";
                        List<QuestionData> parsed = objectMapper.readValue(objJson, new TypeReference<List<QuestionData>>() {});
    ...
    private String removeJsonComments(String jsonText) {
        ...
        for (String line : lines) {
            ...
            if (!cleaned.isEmpty() || result.length() == 0) {
     ...
     * 转换为实体对象
     */
    private PoetryQuizQuestions convertToEntity(BigInteger contentId, QuestionData questionData) {
        PoetryQuizQuestions question = new PoetryQuizQuestions();
        ...
        try {
            question.setQuestionData(objectMapper.writeValueAsString(questionData.questionData));
        } catch (Exception e) {
        ...
        try {
            question.setCorrectAnswer(objectMapper.writeValueAsString(questionData.correctAnswer));
        } catch (Exception e) {
        ...
        if (questionData.hints != null && !questionData.hints.isEmpty()) {
            try {
                question.setHints(objectMapper.writeValueAsString(questionData.hints));
            } catch (Exception e) {
