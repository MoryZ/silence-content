package com.old.silence.content.api;

import org.springframework.cloud.openfeign.SpringQueryMap;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestBody;
import com.old.silence.content.api.dto.PoetryUserLearningRecordCommand;
import com.old.silence.content.api.dto.PoetryUserLearningRecordQuery;
import com.old.silence.content.api.vo.PoetryUserLearningRecordView;
import com.old.silence.web.bind.annotation.PostJsonMapping;
import com.old.silence.web.bind.annotation.PutJsonMapping;
import com.old.silence.web.data.ProjectedPayloadType;

import java.math.BigInteger;
import java.util.Optional;

/**
 * PoetryUserLearningRecord服务接口
 */
interface PoetryUserLearningRecordService {

    @GetMapping(value = "/poetryUserLearningRecords/{id}")
    <T> Optional<T> findById(@PathVariable BigInteger id, @ProjectedPayloadType(PoetryUserLearningRecordView.class) Class<T> projectionType);

    @GetMapping(value = "/poetryUserLearningRecords", params = {"pageNo", "pageSize"})
    <T> Page<T> query(@Validated @SpringQueryMap PoetryUserLearningRecordQuery query, Pageable pageable,
                      @ProjectedPayloadType(PoetryUserLearningRecordView.class) Class<T> projectionType);

    @PostJsonMapping("/poetryUserLearningRecords")
    BigInteger create(@RequestBody @Validated PoetryUserLearningRecordCommand command);

    @PutJsonMapping(value = "/poetryUserLearningRecords/{id}")
    void update(@PathVariable BigInteger id, @RequestBody @Validated PoetryUserLearningRecordCommand command);

    @DeleteMapping("/poetryUserLearningRecords/{id}")
    void deleteById(@PathVariable BigInteger id);
}