 ...
 */
interface PoetryUserFavoriteService {

    @GetMapping(value = "/poetryUserFavorites/{id}")
    <T> Optional<T> findById(@PathVariable BigInteger id, @ProjectedPayloadType(PoetryUserFavoriteView.class) Class<T> projectionType);
    ...
    @PutJsonMapping(value = "/poetryUserFavorites/{id}")
    void update(@PathVariable BigInteger id, @RequestBody @Validated PoetryUserFavoriteCommand command);
